...
void handleAdmin() {
  ...
  html += "<br>";
  html += "<div><strong>Stable Version:</strong> <span id='stable-version'>Checking...</span></div>";
  html += "<div><strong>Beta Version:</strong> <span id='beta-version'>Checking...</span></div>";
  html += "<br>";
  ...
  html += "  ";
  html += "  // Check stable version";
  html += "  fetch('" + String(OTA_VERSION_URL) + "')";
  html += "    .then(response => response.text())";
  html += "    .then(data => {";
  html += "      var stableVersion = data.trim();";
  html += "      document.getElementById('stable-version').innerHTML = stableVersion;";
  html += "      ";
  html += "      // Check beta version";
  html += "      return fetch('" + String(OTA_VERSION_BETA_URL) + "');";
  html += "    })";
  html += "    .then(response => response.text())";
  html += "    .then(data => {";
  html += "      var betaVersion = data.trim();";
  html += "      document.getElementById('beta-version').innerHTML = betaVersion;";
  html += "      ";
  html += "      // Update status based on versions";
  html += "      var stableVersion = document.getElementById('stable-version').innerHTML;";
  html += "      if (stableVersion === currentVersion) {";
  html += "        statusDiv.innerHTML = '<div style=\"background: #28a745; color: white; padding: 8px 16px; border-radius: 6px; font-weight: bold; text-align: center; display: inline-block; margin: 0 auto;\">Up to date (Stable)</div>';";
  html += "      } else if (betaVersion === currentVersion) {";
  html += "        statusDiv.innerHTML = '<div style=\"background: #ffc107; color: black; padding: 8px 16px; border-radius: 6px; font-weight: bold; text-align: center; display: inline-block; margin: 0 auto;\">Running Beta Version</div>';";
  html += "      } else {";
  html += "        statusDiv.innerHTML = '<div style=\"background: #dc3545; color: white; padding: 8px 16px; border-radius: 6px; font-weight: bold; text-align: center; display: inline-block; margin: 0 auto;\">Updates available</div>';";
  html += "      }";
  html += "    })";
  html += "    .catch(err => {";
  html += "      document.getElementById('stable-version').innerHTML = '<span style=\"color: #dc3545;\">Error</span>';";
  html += "      document.getElementById('beta-version').innerHTML = '<span style=\"color: #dc3545;\">Error</span>';";
  html += "    });";
  html += "}";
