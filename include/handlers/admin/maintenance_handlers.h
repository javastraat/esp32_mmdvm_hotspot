/*
 * maintenance_handlers.h - Maintenance Handler Functions for ESP32 MMDVM Hotspot
 *
 * This file contains maintenance-related handler functions extracted from admin.h:
 * - handleResetConfig() - Complete storage reset confirmation page
 * - handleConfirmReset() - Execute complete ESP32 storage reset
 * - handleCleanupPreferences() - Repair and add missing preference keys
 * - handleDownloadUpdate() - Download firmware update from GitHub
 * - handleUploadFirmware() - Upload firmware from local file
 * - handleFlashFirmware() - Flash the prepared firmware
 * - handleExportConfig() - Export complete configuration to file
 * - handleImportConfig() - Import configuration from uploaded file
 * - handleShowPreferences() - Display all stored preferences in web interface
 */

#ifndef HANDLERS_ADMIN_MAINTENANCE_HANDLERS_H
#define HANDLERS_ADMIN_MAINTENANCE_HANDLERS_H
 ...
 * handleResetConfig()
 *
 * Display the complete storage reset confirmation page.
 * Shows warnings about what will be erased and requires confirmation.
 */
 ...
 * handleConfirmReset()
 *
 * Execute complete ESP32 storage reset.
 * Clears all known namespaces, erases NVS partition, clears WiFi credentials,
 ...
 *
 * Repair and add missing preference keys to the NVS storage.
 * Checks all 57 possible preference keys and adds any missing ones
 * with default values from config.h.
 */
 ...
 * Supports stable, beta, and factory versions.
 * Downloads firmware and prepares it for flashing.
 */
 ...
 *
 * Upload firmware from local file.
 * Handles multi-part file upload and prepares firmware for flashing.
 */
 ...
 * handleFlashFirmware()
 *
 * Flash the prepared firmware and restart the device.
 * Firmware must be prepared by either handleDownloadUpdate() or handleUploadFirmware().
 */
 ...
 * handleExportConfig()
 *
 * Export complete configuration to a text file.
 * Includes all DMR, WiFi, system, mode, and MQTT settings.
 ...
 * Parses configuration file and updates all settings.
 * Saves to NVS and restarts device.
 */
 ...
 *
 * Display all stored preferences in the web interface.
 * Shows key name, value, type, and size for all preferences in the mmdvm namespace.
 * Organizes preferences by category (DMR, WiFi, System, Modes, Web, MQTT).
 * Passwords are masked by default with a toggle to show/hide.
 */
...
#endif // HANDLERS_ADMIN_MAINTENANCE_HANDLERS_H
