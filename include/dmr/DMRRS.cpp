...
// Generator Polynomial for RS(12,9)
const uint8_t DMRRS129::POLY[] = {64, 56, 14, 1, 0, 0, 0, 0, 0, 0, 0, 0};

// Galois Field GF(256) exponent table
const uint8_t DMRRS129::EXP_TABLE[] = {
...
const uint8_t DMRRS129::LOG_TABLE[] = {
...
// Galois field multiplication using logarithms
inline uint8_t DMRRS129::gmult(uint8_t a, uint8_t b) {
    if (a == 0 || b == 0)
        return 0;
...
void DMRRS129::encode(const uint8_t* msg, unsigned int nbytes, uint8_t* parity) {
    for (unsigned int i = 0; i < NPAR + 1; i++)
        parity[i] = 0x00;

    for (unsigned int i = 0; i < nbytes; i++) {
        uint8_t dbyte = msg[i] ^ parity[NPAR - 1];
...
// Check Reed-Solomon (12,9) parity
bool DMRRS129::check(const uint8_t* in) {
    uint8_t parity[4];
    encode(in, 9, parity);
