...
All endpoints are served by the built-in HTTP server (default port 80). Most write endpoints require HTTP Basic authentication (`admin` / `pi-star` by default). All API responses use `text/plain` unless otherwise noted.
...
| POST | `/api/factory-reset` | — | Clear NVS namespace and reboot to defaults |
...
|--------|------|-------------------|-------------|
| POST | `/api/send-pocsag` | `ric`, `message`, `type` | Send a POCSAG test message |
...
| GET | `/api/get-mqtt-token` | — | Get current command token (plain text) |
| POST | `/api/save-mqtt-token` | `token=<value>` | Save command token to NVS (empty = disable token check) |

### MQTT Command Channel

When MQTT is enabled the device subscribes to the configured command topic and accepts JSON commands:
...
On connect the device publishes an announce message listing commands and whether a token is required.
...
| GET | `/api/sdcard/files` | — | File listing on SD card (JSON) |
...
| Method | Path | Params | Description |
|--------|------|--------|-------------|
| GET | `/api/snapshots/list` | — | List saved snapshots (JSON) |
...
| Method | Path | Params | Description |
|--------|------|--------|-------------|
| GET | `/api/show-prefs` | — | Display all NVS keys and values in the `mmdvm` namespace (HTML) |
