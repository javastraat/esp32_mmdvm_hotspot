 ...
 *
 * Patched for ESP-IDF v5.3+ (Arduino ESP32 Core 3.x) TCPIP core locking
 * Renamed to WireGuardVPN to avoid collision with the external WireGuard-ESP32 library
 */
#include "WireGuardVPN.h"
...
bool WireGuardVPN::begin(const IPAddress& localIP, const char* privateKey, const char* remotePeerAddress, const char* remotePeerPublicKey, uint16_t remotePeerPort, const char* dns, const char* allowedIps) {
	...
	// TCPIP core locking required on ESP-IDF v5.3+
	LOCK_TCPIP_CORE();
	...
	// Parse allowed IPs (e.g., "0.0.0.0/0" or "10.0.0.0/24")
	if (allowedIps && strlen(allowedIps) > 0) {
		char allowedIpStr[32] = {0};
		strncpy(allowedIpStr, allowedIps, sizeof(allowedIpStr)-1);
		char* slash = strchr(allowedIpStr, '/');
		uint8_t mask = 32;
		if (slash) {
			*slash = '\0';
			mask = atoi(slash+1);
		...
		if (ipaddr_aton(allowedIpStr, &allowed_ip)) {
			peer.allowed_ip = allowed_ip;
			ip_2_ip4(&peer.allowed_mask)->addr = htonl(mask == 0 ? 0 : (0xFFFFFFFF << (32 - mask)));
		} else {
	...
	// Set DNS if provided (user must handle actual DNS config elsewhere if needed)
	// Example: store or log for user code to use
	if (dns && strlen(dns) > 0) {
		ESP_LOGI(TAG, "WireGuard DNS: %s", dns);
	}
